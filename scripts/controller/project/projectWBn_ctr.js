...
var projectWBn_ctr = myApp.controller('projectWBn_ctr', ['$scope', '$rootScope', '$log', '$timeout', '$http', '$location', 'MyProvider', 'publicFunc', 'ioniclocalStorage', '$ionicLoading','$filter', 
function($scope, $rootScope, $log, $timeout, $http, $location, MyProvider, publicFunc, ioniclocalStorage, $ionicLoading,$filter) {
	...
	$scope.GetTreeData = function() {
		var url = MyProvider.domain + "/api/Keyword/GetAllFenZhu?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
		var q = $http.get(url);
		q.success(function(response, status) {
	...
	$scope.D_lineChart = function(timeValue, config) {
		...
		url = MyProvider.domain + "/api/Keyword/GetTimeLinkCount?categoryIds=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId + "&startTime=" + $scope.startTime + "&endTime=" + $scope.endTime + "&percent=" + $scope.data.percent + "&topNum=" + $scope.data.topNum + "&sumNum=" + 10 + "&timeInterval=" + timeInterval;
		var q = $http.get(url);
		q.success(function(response, status) {
	...
	$scope.D_GetBubbleList = function() {
		...
		var url = MyProvider.domain + "api/Keyword/GetAllDomainCategory?prjId=" + $rootScope.getProjectId;
		var q = $http.get(url);
		q.success(function(response, status) {
			...
			var url = MyProvider.domain + "/api/Keyword/GetDomainStatis?categoryIds=" + $scope.categoryId + "&projectId=" + $rootScope.getProjectId;
			var q = $http.get(url);
			q.success(function (response, status) {
				myChart.hideLoading();
			    console.log(response);
	...
	$scope.quanzhongtu = function(treeNodeId) {
		...
		var url = MyProvider.domain + "/api/Jieba/BaiduExtract?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
		var q = $http.get(url);
		q.success(function(response, status) {
			console.log(response);
	...
	$scope.cipintongji = function(treeNodeId) {
		...
		var url = MyProvider.domain + "/api/Jieba/BaiduFrequency?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
		var q = $http.get(url);
		q.success(function(response, status) {
			myChart.hideLoading();
			myChart2.hideLoading();
	...
	$scope.searchLinkByTime = function(time) {
		...
		if (re.test($scope.data.searchTime) || $scope.data.searchTime == '') {
			...
			var url = MyProvider.domain + "/api/Keyword/GetTimeLinkList?usr_id=" + $rootScope.userID + "&prjId=" + $rootScope.getProjectId + "&categoryId=" + $scope.treeNodeId1 + "&pubTime=" + time;
			var q = $http.get(url);
			q.success(function(response, status) {
				console.log(response);
				if (response.Count == 0) {
	...
	$scope.GetTimeLinkList = function(treeNodeId) {
		var url = MyProvider.domain + "/api/Keyword/GetTimeLinkList?usr_id=" + $rootScope.userID + "&prjId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId + "&pubTime=";
		var q = $http.get(url);
		q.success(function(response, status) {
			console.log(response);
			$scope.TimeLinkList = response.Result;
	...
	$scope.getkeyword = function(treeId, treeNode) {
			...
			var url = MyProvider.domain + "/api/Keyword/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + treeNode.id + "&status=" + $scope.getNotRemovedKw;
			var q = $http.get(url);
			q.success(function(response, status) {
	...
	$scope.getkeyword1 = function(id) {
		var url = MyProvider.domain + "/api/Keyword/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + id + "&status=" + $scope.getNotRemovedKw;
		var q = $http.get(url);
		q.success(function(response, status) {
	...
	$scope.GetBaiduLevelLinks = function() {
			var url = MyProvider.domain + "/api/Keyword/GetLevelLinks?user_id=" + $rootScope.userID + "&categoryId=" + $rootScope.categoryId + "&projectId=" + $rootScope.getProjectId + "&keywordId=" + $rootScope.BaidukeywordId + "&Title=" + $scope.Title +
...
$scope.GetBaiduLevelLinksMore=function(){
	...
	$scope.GetNameStatistic = function (categoryIds) {
	    ...
	    var url = "api/Media/GetNameStatistic?categoryIds=" + categoryIds + "&projectId=" + $rootScope.getProjectId ;
	    var q = $http.get(url);
	    q.success(function (response, status) {
    ...
    //前端分页
	$scope.GetNameStatisticPage = {
	    page: 1,
	    pagesize: 10
	}
	$scope.GetNameStatisticCP = function () {
	    var start = ($scope.GetNameStatisticPage.page - 1) * $scope.GetNameStatisticPage.pagesize;
	    var end = start + 10;
	    $scope.GetNameStatisticList = $scope.GetNameStatisticList1.slice(start, end);

	}

	$scope.GetNameStatistic();
