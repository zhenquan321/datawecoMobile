...
var project_ctr = myApp.controller('project_ctr', ['$scope', '$rootScope', '$log', '$timeout', '$http', '$location', 'MyProvider', 'publicFunc', 'ioniclocalStorage', '$ionicLoading','$ionicListDelegate','$ionicScrollDelegate',
function($scope, $rootScope, $log, $timeout, $http, $location, MyProvider, publicFunc, ioniclocalStorage, $ionicLoading,$ionicListDelegate,$ionicScrollDelegate) {
    ...
    $scope.GetProjects = function () {
        ...
        q.success(function (response, status) {
            ...
            $timeout(function(){    
            	$scope.PushData('modal',$rootScope.projectsList);
            },10)
    ...
    $scope.GetMyShareProjects = function () {
        ...
        q.success(function (response, status) {
            ...
            console.log(response);
            $timeout(function(){    
            	 $scope.PushData('myShareModal',$scope.ShareProjects);
...
//              },20)
    ...
    $scope.GetShareToMeProjects = function () {
        ...
        q.success(function (response, status) {
            ...
            //项目链接数变化图
            $timeout(function(){            		
            	$scope.PushData('shareMeModal',$scope.ToMeProjects);
            },10)
    ...
    //生成JQ SLine
    $scope.PushData=function(modal,list){
    	
        for (var aa = 0; aa < list.length; aa++) {
        	var countList=list[aa].CountList;
        	var bb=modal +aa;
            var cc = "#" + modal +aa;
            var dd= document.getElementById(bb);
            if(dd){
            	$scope.PushJQSLine(cc,countList);
            }else{
            	$timeout(function(){
            		$scope.PushData(modal,list);
            	},10)
            }
        }  	
    }
    
    $scope.PushJQSLine=function(cc,list){
    	     $(cc).sparkline(list, {
                type: 'line',
                width: '85',
                height: '35',
...
//加载更多
 $scope.loadMore = function() {
  ...
  $scope.$on('stateChangeSuccess', function() {
		...
		if($scope.selectModal==1){
			$scope.pagesize4=$scope.pagesize4+10;
    		$scope.GetProjects();
		}else if($scope.selectModal==2){
			$scope.pagesize_shareMe=$scope.pagesize_shareMe+10;
    		$scope.GetShareToMeProjects();
		}else if($scope.selectModal==3){
			$scope.pagesize_share=$scope.pagesize_share+10;
			$scope.GetMyShareProjects();
		}
